import {
  faFacebookF,
  faTwitter,
  faLinkedinIn,
  faGithub,
} from '@fortawesome/free-brands-svg-icons';

export default {
  name: 'Raul Max.',
  typed: ['Backend Developer.', 'Frontend Developer.', 'Coffee Lover.'],
  socialLinks: [
    {
      name: 'Facebook',
      url: 'https://www.facebook.com/raulmax319/',
      className: faFacebookF,
    },
    {
      name: 'Twitter',
      url: 'https://www.twitter.com/raulmax319',
      className: faTwitter,
    },
    {
      name: 'Linkedin',
      url: 'https://www.linkedin.com/in/raulmax319/',
      className: faLinkedinIn,
    },
    {
      name: 'GitHub',
      url: 'https://github.com/raulmax319',
      className: faGithub,
    },
  ],
  thingsList: [
    [
      {
        text: 'React',
        icon: 'SiReact',
      },
      {
        text: 'JavaScript',
        icon: 'SiJavascript',
      },
      {
        text: 'CSS',
        icon: 'SiCss3',
      },
      {
        text: 'MySQL',
        icon: 'SiMysql',
      },
      {
        text: 'Redux',
        icon: 'SiRedux',
      },
    ],
    [
      {
        text: 'Node',
        icon: 'SiNodeDotJs',
      },
      {
        text: 'TypeScript',
        icon: 'SiTypescript',
      },
      {
        text: 'Git',
        icon: 'SiGit',
      },
      {
        text: 'MongoDB',
        icon: 'SiMongodb',
      },
      {
        text: 'Yarn',
        icon: 'SiYarn',
      },
    ],
  ],
  aboutMe: 'About Me',
  aboutDetails: 'Full-Stack developer',
  details: "I'm Raul Max ty",
  cvLink:
    'https://1drv.ms/b/s!AtiUcFNLq43kg4oOi1gYyyYy5gxO6g?e=HtcwyY',
  profilePic:
    'https://avatars.githubusercontent.com/u/17752968?v=4',
};
